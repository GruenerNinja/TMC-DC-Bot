...
async def on_member_join(member):
    ...
    rank_role = discord.utils.get(client.guilds[0].roles, name='Rank')
    game_role = discord.utils.get(client.guilds[0].roles, name='Game')
    ping_role = discord.utils.get(client.guilds[0].roles, name='Pingrollen')
    content_creator_role = discord.utils.get(client.guilds[0].roles, name='ContentCrator')
    prefix_rank_role = discord.utils.get(client.guilds[0].roles, name='Prefix Rank')
    other_role = discord.utils.get(client.guilds[0].roles, name='Other')
...
# Define the options for the slash command
options = [
    create_option(
        name="role",
        description="The role to add or remove",
        option_type=8,  # Type 8 = Role
        required=True,
    ),
    create_option(
        name="action",
        description="Whether to add or remove the role",
        option_type=3,  # Type 3 = String
        required=True,
        choices=[
            {"name": "Add", "value": "add"},
...
@slash.slash(
    name="react",
    description="Add or remove a role by reacting to a message",
    options=options,
)
